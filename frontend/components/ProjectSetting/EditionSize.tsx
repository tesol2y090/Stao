...
const EditionSizeComponent = ({
  ...
  const [editionSize, setEditionSize] = useState<number>(0)
  ...
  const { config: proposConfig } = usePrepareContractWrite({
    ...projectContract,
    functionName: "proposeEditionSize",
    args: [
      editionSize,
      staoImplementContract.interface.encodeFunctionData("setEditionSize", [
        editionSize,
      ]),
      description,
    ],
  ...
  const [proposalStat, proposalVote, editionSizeContract, maxContributors] =
    governorData || [
      "0",
        ...
        <>
          {proposalId.toString() === "0" ? (
            ...
            <>
              ...
              <Row>
                <span>Edition Size:</span>
                <span>{editionSizeContract.toString()}</span>
              </Row>
              <Row>
                <span>Vote For:</span>
                <span>
          ...
          {proposalId.toString() === "0" ? (
            <>
              <div style={{ color: "yellow", textAlign: "center" }}>
                <h5>"Project Owner will proposing Edition size prarameter"</h5>
              </div>
            </>
          ) : (
            <>
...
export default EditionSizeComponent
